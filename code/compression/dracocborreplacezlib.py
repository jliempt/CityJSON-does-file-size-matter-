...
def compress_dracocborreplacezlib(cmpath, comprout, dataset, cm=None):
    if cm == None:
        cm_file = open(cmpath)
    ...
    out = {"geom": drc, "cm": cm, "attr": encoded}
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        out = comprout + "dracocborreplacezlib/" + dataset + "_dracocborreplacezlib.cbor" 
        
        for i in range(3):   
            start = ms_time()
            compress_dracocborreplacezlib(file, out, dataset)
            end = ms_time()
    ...
    create_report("dracocborreplacezlib", dataset, benchmarks, columns)
