...
def compress_originalcborreplacezlib(cmpath, comprout, cm=None):
    if cm == None:
        cm_file = open(cmpath)
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        out = comprout + "originalcborreplacezlib/" + dataset + "_originalcborreplacezlib.cbor" 
        
        for i in range(3):   
            start = ms_time()
            compress_originalcborreplacezlib(file, out)
            end = ms_time()
    ...
    create_report("originalcborreplacezlib", dataset, benchmarks, columns)
