...
def compress_originalcborzlib(cmpath, comprout, cm=None):
    if cm == None:
        cm_file = open(cmpath)
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        #if dataset != "hdb":
        #    continue
        columns.append("originalcborzlib," + dataset)
        out = comprout + "originalcborzlib/" + dataset + "_originalcborzlib.cbor" 
        
        for i in range(10):   
            start = ms_time()
            compress_originalcborzlib(file, out)
            end = ms_time()
    ...
    create_report("originalcborzlib", dataset, benchmarks, columns)