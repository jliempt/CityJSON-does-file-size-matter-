...
def compress_dracozlib(cmpath, comprout, dataset, cm=None):
    if cm == None:
        cm_file = open(cmpath)
    ...
    # because drc files of regular datasets can be used
    #if "_" in dataset:
    ...
    delim = "DRCDLM" # compute shortest string that is not yet in the file? then put that as first word to select as delimiter
    
    cout_s = ""
    cout_s += str(len(delim) + 1)
    cout_s += delim
    cout_s += json.dumps(cm.j)
    cout_s += delim
    cout_b = cout_s.encode()
    compressed = zlib.compress(cout_b)
    # can't zlib drc because drc is loaded separately...
    compressed += drc
...
def compression_factors(cmpath, dataset, method):
    ...
    compressedsize = os.path.getsize("../../datasets/" + method + "/" + dataset + "_" + method + ".zlib" )
...
if __name__ == "__main__":
    ...
    create_report("dracozlib", dataset, benchmarks, columns)
